...
namespace ConsoleApp.Socks
{
	public class TransferAdapter
	{
		...
		public bool Start()
		{
			...
			loopTask = Task.Run(() =>
			{
				while (!this.breakLoopTrace&&this.SocksClientConnected)
				{
					LoopTran();
				}
			...
			loopTask.ContinueWith((x, y) =>
			{
				var args = (y as List<object>);
				(args[0] as Socket).Close();
				(args[1] as Socks5Client)?.Close();
				logger.Info("closing socket and client");
		...
		private void LoopTranClient()
		{
			...
			logger.Info("client prepare to receive");
			var revResult = opendSocks5Client.Receive(buffer, 0, 4096);
			if (revResult > 0)
			{
